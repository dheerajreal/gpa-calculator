...
if (!Number.isInteger(subs) || subs < 0) {
    document.write(
        "<h2 class=error> Enter positive  numeric value for subject!</h2>"
    );
    errorMessage();
}
//bunch of variables
...
for (i = 0; i < subs; i++) {
    ...
    if (!Number.isInteger(marks) || marks > 100 || marks < 0) {
        document.write(
            "<h2 class=error> Only numeric values between 0 and 100 are allowed!</h2>"
        );
        errorMessage();
    }
...
document.writeln(
    "<h2>The average marks are :" + (sum / subs).toFixed(2) + "</h2>"
);
